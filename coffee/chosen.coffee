class Chosen extends AbstractChosen
  ...
  results_build: ->
    ...
    if @is_multiple
      @search_choices.querySelectorAll("li.search-choice").forEach (el) ->
        el.remove()
  ...
  set_label_behavior: ->
    @form_field_label = @form_field.parents("label")[0] # first check for a parent label
    if not @form_field_label and @form_field.id.length
      @form_field_label = document.body.querySelector "label[for='#{@form_field.id}']" #next check for a for=#{id}

    if @form_field_label
      @form_field_label.addEventListener 'click', (evt) => if @is_multiple then this.container_mousedown(evt) else this.activate_field()
  ...
  search_results_mouseup: (evt) ->
    target = if evt.target.classList.contains "active-result" then evt.target else evt.target.parents(".active-result")[0]
    if target
  ...
  search_results_mouseout: (evt) ->
    this.result_clear_highlight() if evt.target.classList.contains "active-result" or evt.target.parents(".active-result")[0]
  ...
  results_reset_cleanup: ->
    @current_selectedIndex = @form_field.selectedIndex
    @selected_item.querySelectorAll("abbr").forEach (el) ->
      el.remove()
  ...
  winnow_results_set_highlight: ->
    selected_results = if not @is_multiple then @search_results.querySelector(".result-selected.active-result") else null
    do_high = if selected_results then selected_results else @search_results.querySelector(".active-result")
  ...
  no_results_clear: ->
    @search_results.querySelectorAll(".no-results").forEach (el) ->
      el.remove()
